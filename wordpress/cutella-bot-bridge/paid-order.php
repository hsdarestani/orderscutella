<?php
// Paid Vestaland marketplace order sync for Cutella Bot Bridge.
if (!defined('ABSPATH')) { exit; }

function cbb_vestaland_money_to_toman($value) {
    $amount = (float) $value;
...
function cbb_vestaland_clean_address($raw) {
...
function cbb_vestaland_find_existing_order($receipt, $intent) {
...
function cbb_create_paid_vestaland_order($payload) {
    ...
    $address = cbb_vestaland_clean_address($payload['address'] ?? array());
    ...
    $existing = cbb_vestaland_find_existing_order($receipt, $intent);
    if ($existing) {
        return array(
    ...
    foreach ($items as $row) {
        ...
        $current_price = cbb_vestaland_money_to_toman($product->get_price());
    ...
    if ($computed !== $expected) { throw new Exception('Vestaland paid amount does not match Cutella items.'); }
    ...
    try {
        ...
        $calculated = cbb_vestaland_money_to_toman($order->get_total());
...
function cbb_vestaland_paid_order_route() {
    if (!function_exists('cbb_v2_request') || !cbb_v2_request()) { return; }
    $op = isset($_GET['o']) ? sanitize_key(wp_unslash($_GET['o'])) : '';
    if ($op !== 'create_paid_order') { return; }
    list($authorized_op, $payload) = cbb_v2_authorize();
    if ($authorized_op !== 'create_paid_order') { cbb_fail('Invalid paid-order operation.', 400); }
    try {
        cbb_no_cache();
        wp_send_json_success(cbb_create_paid_vestaland_order($payload));
    } catch (Throwable $e) {
        cbb_fail($e->getMessage(), 500);
    }
    exit;
}
add_action('plugins_loaded', 'cbb_vestaland_paid_order_route', PHP_INT_MAX - 100);

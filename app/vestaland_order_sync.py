...
def hamoon_status(receipt):
    ...
    req = urllib.request.Request(url, headers={
        ...
        'User-Agent': 'OrdersCutellaVestalandSync/1.0',
    })
    try:
...
class VestalandOrderSyncHTTP(HealthHandler):
    ...
    def do_GET(self):
        if urlparse(self.path).path.rstrip('/') == '/vestaland-order-sync/health':
            return self._json(200, {'ok': True, 'service': 'orderscutella-vestaland-order-sync', 'store': STORE})
        return super().do_GET()
